 ...
 module.exports = {
	 ...
	 */
	 parseCssString: function(contents, options) {
		...
		// Process comments
		var processComment = function(comment, prefix) {
			prefix = prefix || '';
			var strCss = prefix + '/*' + comment.comment + '*/' + "\n";

			return strCss;
		};

		// Process declaration
		var processDeclaration = function(declaration, prefix) {
			prefix = prefix || '';
			var strCss = prefix + declaration.property + ': ' + declaration.value + ';' + "\n";

			return strCss;
		};

		// Check declarations type
		var commentOrDeclaration = function(declarations, prefix) {
			var strCss = '';
			prefix = prefix || '';

			if (declarations.type === 'declaration') {
				strCss += processDeclaration(declarations, prefix);
			} else if (declarations.type === 'comment') {
				strCss += ' ' + processComment(declarations, prefix);
			}
		...
		var processRule = function(rule, prefix) {
			prefix = prefix || '';
			var strCss = '';

			strCss += prefix + rule.selectors.join(',') + ' {' + "\n";
			rule.declarations.forEach(function(rules) {
				strCss += commentOrDeclaration(rules, prefix + "\t");
			});
			strCss += prefix + '}' + "\n";
		...
		// Check rule type
		var commentOrRule = function(rule, prefix) {
			var strCss = '';
			prefix = prefix || '';

			if (rule.type === 'rule') {
				strCss += processRule(rule, prefix);
			} else if (rule.type === 'comment') {
				strCss += processComment(rule, prefix);
			} else if (rule.type === 'media') {
				strCss += processMedia(rule, prefix);
			} else if (rule.type === 'page') {
				strCss += processPage(rule, prefix);
			} else if (rule.type === 'supports') {
				strCss += processSupports(rule, prefix);
			} else if (rule.type === 'keyframes') {
				strCss += processKeyframes(rule, prefix);
			}
		...
		// Check keyframe type
		var commentOrKeyframe = function(frame, prefix) {
			prefix = prefix || '';
			var strCss = '';

			if (frame.type === 'keyframe') {
				strCss += prefix + frame.values.join(',') + ' {' + "\n";
				frame.declarations.forEach(function(declaration) {
					strCss += commentOrDeclaration(declaration, prefix + "\t");
				});
				strCss += prefix + '}' + "\n";
			} else if (frame.type === 'comment') {
				strCss += prefix + processComment(frame, prefix);
			}
		...
		// Process media queries
		var processMedia = function(media, prefix) {
			prefix = prefix || '';
			var strCss = '',
			query = media.rule;
			...
			if (query) {
				strCss += prefix + '@media ' + query + ' {' + "\n";
				media.rules.forEach(function(rule) {
					strCss += commentOrRule(rule, prefix + "\t");
				});
				strCss += prefix + '}' + "\n";
			}
		...
		// Process keyframes
		var processKeyframes = function(key, prefix) {
			prefix = prefix || '';
			var strCss = prefix + '@' + (typeof key.vendor !== 'undefined' ? key.vendor : '') + 'keyframes ' + key.name + ' {' + "\n";

			key.keyframes.forEach(function(keyframe) {
				strCss += commentOrKeyframe(keyframe, prefix + "\t");
			});
		...
		// Process @page
		var processPage = function(supports) {
			var strCss = '@page {' + "\n";

			supports.declarations.forEach(function (declaration) {
				strCss += "\t" + declaration.property + ':' + declaration.value + ';' + "\n";
			});
		...
		// Process @supports
		var processSupports = function(supports, prefix) {
			var strCss = '',
			query = supports.rule;
			prefix = prefix || '';

			if (!query && supports.supports) {
				query = supports.supports;
			}

			if (query) {
				strCss += prefix + '@supports ' + query + ' {' + "\n";
		...
		cssJson.stylesheet.rules.forEach(function(rule) {
			...
			if (rule.type === 'media') {
				...
				processedCSS.page.push(rule);
			} else if (rule.type === 'supports') {
				// Create 'id' based on the query (stripped from spaces and dashes etc.)
				var strSupports = rule.supports.replace(/[^A-Za-z0-9]/ig, '');

				// Create an array with all the supports queries with the same 'id'
				var item = processedCSS.supports.filter(function(element) {
					return (element.val === strSupports);
				});

				// If there are no supports queries in the array, define details
				if (item.length < 1) {
					var supportsObj = {};
					supportsObj.sortVal = parseFloat(rule.supports.match(/\d+/g));
					supportsObj.rule = rule.supports;
					supportsObj.val = strSupports;
					supportsObj.rules = [];

					processedCSS.supports.push(supportsObj);
				}

				// Compare the query to other queries
				var i = 0,
				matched = false;
				processedCSS.supports.forEach(function(elm) {
					if (elm.val === strSupports) {
						matched = true;
					}
					if (!matched) {
						i++;
				...
				// Push every merged query
				rule.rules.forEach(function(supportsRule) {
					if (supportsRule.type === 'rule' || 'comment') {
						processedCSS.supports[i].rules.push(supportsRule);
					}
				});
			...
			} else if (rule.type === 'rule' || rule.type === 'comment') {
...
var i = 0;
		...
		// Function to output page CSS
		var outputPage = function(page){
			page.forEach(function (page) {
				strStyles += processPage(page);
			});
		};

		// Function to output supports CSS
		var outputSupports = function(supports){
			supports.forEach(function (supports) {
				strStyles += processSupports(supports);
			});
		};
		...
		// Check if keyframes were processed and print them
		if (processedCSS.page.length !== 0) {
			outputPage(processedCSS.page);
		}
